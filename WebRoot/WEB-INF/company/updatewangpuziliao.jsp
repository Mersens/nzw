...
<jsp:include page="/userCommonCenter/gotoLeft" />
        ...
        <div class="right">
            <p class="r_tit">店铺基本信息设置</p>
  		  </div>
    <div class="mj_rm" >
        <div class="inf">
            <div class="inf_l" style="width:91px;"><span class="red">*</span>旺铺名称：</div>
            <div class="inf_r" style="width:749px;">
                <input class="text_inp" id="dianpuname" name="dianpuname" style="width:370px;" type="text" value="${dianpu.dianpuname }" disabled="disabled"/><span class="color:#c27200" id="spShopName"></span>
            </div>
        </div>
           <div class="inf">
               ...
               <div style="float:left;"> 
                 <c:forEach items="${typelist }" var="t" varStatus="status">
                     <input class="radio"  id="type${status.index }" name="type${status.index+1 }" type="radio" value="${t.typeid }"  disabled="disabled" <c:if test="${dianpu.dianputype.typeid eq t.typeid }">checked="checked"</c:if>  />${t.typename }&nbsp;
                 </c:forEach>
               </div>
        ...
        <div class="inf">
            ...
            <c:forEach items="${leimulist }" var="l" varStatus="status">
             <input class="zliao_ra"  id="leimu${status.index }" type="checkbox" name="leimu"  value="${l.categoryId }" <c:forEach items="${dianpuleimu }" var="d"><c:if test="${d.leimu.categoryId eq l.categoryId}"> checked="checked"</c:if> </c:forEach>  />${l.name }&nbsp;&nbsp;
            </c:forEach>
        ...
        <div class="inf">
            <div class="inf_l" style="width:91px;">旺铺简介：</div>
             <div class="fitem" >
                 <textarea name="dianpujieshao" id="dianpujieshao"  class="easyui-kindeditor" style="width: 65%; height: 350px; visibility: hidden;">${dianpu.dianpujieshao }</textarea>
             </div>
        </div>
        ...
        <div class="inf">
            ...
            <div class="inf_r">
                <input type="submit" id="btn" value="保存并提交" class="tj_but" onclick="updatewangpu()" />
            </div>
        </div>
